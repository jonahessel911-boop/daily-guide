...
async function sendPurchaseNotification(intent) {
  ...
  const product = meta.product || 'Slaap Beter Slapen';
  ...
  if (meta.shipping_postal_code || meta.shipping_house_number) {
    shippingLines.push(
      `Adres: ${meta.shipping_postal_code || ''} ${meta.shipping_house_number || ''}`.trim()
    );
  }
  ...
  return sendEmail({
    to: process.env.POSTMARK_NOTIFY_TO,
    subject,
    htmlBody,
    textBody,
  });
