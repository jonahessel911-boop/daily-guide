...
function aggregateRows(events) {
  ...
  for (const e of events) {
    ...
    const row = ensure(productSlug, e.country, landerSlug);
...
async function getStats({ from, to, product } = {}) {
  ...
  const allRows = aggregateRows(data || []);
  const products = [...new Set(allRows.map((r) => r.product_slug))]
    .sort()
    .map((slug) => ({ slug, name: productName(slug) }));

  const selected = product && product !== 'all' ? product : null;
  const rows = selected ? allRows.filter((r) => r.product_slug === selected) : allRows;
