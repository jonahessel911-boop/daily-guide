...
function buildPurchaseEvent(intent) {
  ...
  return {
    ...
    metadata: {
      ...
      shipping_house_number: meta.shipping_house_number || null,
      shipping_house_addition: meta.shipping_house_addition || null,
      ...
      cameras: meta.cameras || null,
      printers: meta.printers || null,
      hearings: meta.hearings || null,
...
async function sendPurchaseCapiOnce(intent, context = {}) {
  ...
  if (!meta.fbc) {
    console.warn(
      'Meta CAPI purchase zonder fbc (click-id) — attributie naar campagne kan misgaan:',
      intent.id,
      meta.product_slug || 'unknown'
    );
  }
  ...
  const result = await sendPurchaseEvent({
    ...
    contentIds: meta.product_slug ? [meta.product_slug] : undefined,
    contentName: meta.product || undefined,
    contentType: 'product',
  });
